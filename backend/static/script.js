...
recordButton.addEventListener('click', async function recordHandler() {
    ...
    try {
        ...
        mediaRecorder.start();
        recordButton.disabled = false; // Enable button for second click
        recordButton.innerHTML = '<img src="/static/stop_button.png" alt="Stop" class="button-icon">';
        recordingStatus.textContent = 'Recording... (Click again to stop)';

        // Auto-stop after 15 seconds if not stopped manually
        setTimeout(() => {
            if (mediaRecorder && mediaRecorder.state !== 'inactive') {
                recordingStatus.textContent = 'Time limit reached. Processing...';
                mediaRecorder.stop();
            }
        }, 15000);
