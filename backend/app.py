...
def clear_collection(device_id, character):
    collection_name = f"{device_id}_{character}"  # Use device_id instead of IP
    print(f"Clearing Firestore '{collection_name}' collection...")
    docs = db.collection(collection_name).stream()
    for doc in docs:
        doc.reference.delete()

def clear_storage(device_id, character):
    prefix = f"audio/{device_id}_{character}/"  # Use device_id instead of IP
    print(f"Clearing Firebase Storage '{prefix}' folder...")
    blobs = bucket.list_blobs(prefix=prefix)
    for blob in blobs:
        blob.delete()
...
def get_conversation(device_id, character):
    collection_name = f"{device_id}_{character}"  # Use device_id instead of IP
    conversation_ref = db.collection(collection_name).order_by('timestamp')
    return [doc.to_dict() for doc in conversation_ref.stream()]
...
def character_chat(character):
    if character not in character_data:
        return "Character not found", 404
    device_id = request.args.get('device_id')  # Get device_id from query parameter
    if not device_id:
        return jsonify({"error": "No device_id provided"}), 400
    conversation = get_conversation(device_id, character)
    return render_template('character_template.html',
                         character_id=character,
                         character_name=character_data[character]['name'],
                         character_title=character_data[character]['title'],
                         character_description=character_data[character]['description'],
                         conversation=conversation,
                         ip_address=device_id)  # Pass device_id as ip_address to template
...
def process_audio():
    ...
    try:
        ...
        recorded_blob_path = f"audio/{device_id}_{character}/recorded_audio_{timestamp}.wav"  # Use device_id
        recorded_blob = bucket.blob(recorded_blob_path)
        recorded_blob.upload_from_filename(converted_audio_path)
        recorded_blob.make_public()
        recorded_audio_url = recorded_blob.public_url
        print(f"Uploaded recorded audio to {recorded_blob_path}: {recorded_audio_url}")
        ...
        synthesized_blob_path = f"audio/{device_id}_{character}/synthesized_audio_{timestamp}.mp3"  # Use device_id
        synthesized_blob = bucket.blob(synthesized_blob_path)
        synthesized_blob.upload_from_filename(synthesized_audio_path)
        synthesized_blob.make_public()
        synthesized_audio_url = synthesized_blob.public_url
        ...
        conversation_ref = db.collection(f"{device_id}_{character}")  # Use device_id
        conversation_ref.add({
            'user_input_id': f"user_input_{timestamp}",
            'timestamp': firestore.SERVER_TIMESTAMP,
            'user_input': transcript,
...
def process_text():
    ...
    try:
        data = request.get_json()
        device_id = data.get('device_id')  # Get device_id from JSON body
        if not device_id:
            return jsonify({"error": "No device_id provided"}), 400
        text = data.get('text', '').strip()
        character = data.get('character')
...
def clear_chat():
    try:
        data = request.get_json()
        device_id = data.get('device_id')  # Use device_id instead of ip_address
        character = data.get('character')
        if not device_id or not character:
            return jsonify({"error": "Missing device_id or character"}), 400
        
        clear_collection(device_id, character)
        clear_storage(device_id, character)
        print(f"Cleared chat for Device: {device_id}, Character: {character}")
        return jsonify({"message": "Chat cleared"}), 200
    except Exception as e:
        print(f"Error in clear_chat: {str(e)}")
        return jsonify({"error": str(e)}), 500
